...
4. 在“单位属性”“建筑”“玩家属性”或“高级功能”页面修改数值；“建筑”页面只填写“矿产（倍数）”和“仓库（倍数）”，两项都是全部批量修改。进入建筑页时会自动读取当前存档，页面会显示已读取的资源点和仓库数量；鼠标悬停在输入框上可查看存档中的当前上限。所有修改暂存后，点击顶部“保存修改”统一写入。

“建筑”页面不提供单项选择。矿产倍数覆盖全部已识别资源点；仓库倍数覆盖全部辎重库、粮仓和军械库，并且只放大存档中已经存在的储存类别。输入框里的数字是“本次修改倍数”，不是固定默认容量：1 倍表示不修改，2 倍表示在读取到的当前上限基础上放大为 2 倍。
...
* 游戏运行中图形界面的“保存修改”会被禁用；请完全退出游戏后再保存，避免游戏内存中的建筑数据覆盖仓库容量。
