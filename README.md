...
Windows x64 存档修改工具。打开一个《烽沙》存档槽后，可以批量修改单位、资源和玩家属性，并在写入前自动备份、写入后完整校验。
...
预览不写文件：在修改命令末尾加 `--dry-run`。`--yes` 只跳过确认，不会跳过备份和回读校验。

## 当前边界

* 修改对象是当前存档中已经存在的区域；以后新生成的民夫不会自动继承旧修改。
* “全部自有兵种”按已识别的单位类型筛选，不包含野兽、建筑和城防设施。
* 资源设为 99,999 是一次性存档修改，采集后仍可能减少，不是常驻内存锁定。
