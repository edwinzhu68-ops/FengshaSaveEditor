...
| 仓库读取 | 读取全部辎重库、粮仓、军械库；只显示存档里已保存的物品上限 |
...
4. 在“单位属性”“建筑”“玩家属性”或“高级功能”页面修改数值；“建筑”页面的“矿产（几倍）”会批量处理全部已识别矿产。倍数只能填写整数：1=不修改，2=当前值×2，5=当前值×5。所有修改暂存后，点击顶部“保存修改”统一写入。

“建筑”页面不提供单项选择。仓库界面显示的“物资仓库 2,000”是游戏运行时计算的总容量；当前存档只保存仓库物品列表和单项上限，没有可安全写入的总容量字段。因此 v36 已移除仓库倍数写入，避免把物品上限误当成总容量。
...
* 仓库总容量（例如游戏界面的 2,000）当前只读。存档中的 `StorageMaxItems` 是物品单项上限，修改它不会改变总容量；仓库写入命令已停用。
...
* 游戏运行中图形界面的“保存修改”会被禁用；请完全退出游戏后再保存，避免游戏内存中的数据覆盖存档。
